...
async function initCalendar() {
  try {
      ...
      flatpickr("#calendar", {
          locale: 'pt',
          altInput: true,          // cria um input 'bonito' separado
          altFormat: "d-m-Y",  // Exibição do usuario
          dateFormat: "Y-m-d", // Formato interno
          enable: availableDates, // aceita datas em formato ISO (Y-m-d)
          locale: {
              firstDayOfWeek: 1 // começa segunda-feira
          },
          defaultDate: localStorage.getItem('data') || availableDates[0],        // sem data pré-selecionada
          onChange: function(selectedDates, dateStr, instance) {
            // Ao selecionar uma data, você pode realizar ações adicionais
...
const calendario = flatpickr("#calendar", {
  defaultDate: localStorage.getItem("data") || new Date(),
  onChange: function (selectedDates, dateStr) {
    localStorage.setItem("data", dateStr);
    carregarDadosDoDia(dateStr);
  }
});
